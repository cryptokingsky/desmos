...
func (suite *KeeperTestSuite) Test_queryProfiles() {
	...
	for _, test := range tests {
		...
		suite.Run(test.name, func() {
			...
			for _, profile := range test.storedAccounts {
			...
			var returned []types.Profile
			err = suite.legacyAminoCdc.UnmarshalJSON(result, &returned)
			suite.Require().NoError(err)
			suite.Require().Equal(test.expProfiles, returned)
		})
	}
}
